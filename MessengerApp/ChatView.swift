        ...
        NavigationStack {
            VStack(spacing: 0) {
                        ...
                        ForEach(messages) { message in
                            HStack {
                                ...
                                .contextMenu {
                                    ...
                                    Button("Реакция ❤️", systemImage: "heart") {
                                        session.react(message, in: chat, emoji: "❤️")
                                    }
                                    if message.isMine {
                                        ...
                                        Button("Удалить", systemImage: "trash", role: .destructive) {
                                            session.deleteMessage(message, in: chat)
                                        }
                                    }
                                }
                ...
                HStack(spacing: 10) {
                    ...
                    Menu {
                        Button("Фото или видео", systemImage: "photo") {
                            showFilePicker = true
                        }
                        Button("Файл", systemImage: "doc") {
                            showFilePicker = true
                        }
                    } label: {
            ...
            .alert(
                ...
                TextField(
                    "Текст",
                    text: Binding(
                        get: { editingMessage?.text ?? "" },
                        set: { editingMessage?.text = $0 }
                    )
                )
                Button("Сохранить") {
                    if let message = editingMessage {
                        session.updateMessage(message, in: chat, text: message.text)
                    }
                    editingMessage = nil
                }
